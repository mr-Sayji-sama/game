...
namespace game
{
    ...
    public class Game
    {
        ...
        public static void task1()
        {
            ...
            Score score=game.getScore(36667);
        }
        ...
        public Score getScore(int offset)
        {
            ...
            GameStamp cnStamp = this.gameStamps[0];
            foreach (GameStamp stamp in this.gameStamps)
            {
                if (stamp.offset > offset)
                {
                    Console.WriteLine($"{cnStamp.offset}: {cnStamp.score.home}-{cnStamp.score.away}");
                    return new Score() { home = cnStamp.score.home, away = cnStamp.score.away };
                }
                cnStamp = stamp;
            }
            return new Score() { home = cnStamp.score.home, away = cnStamp.score.away };
